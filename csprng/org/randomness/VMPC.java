 ...
 * <code>org.bouncycastle.crypto.prng.VMPCRandomGenerator</code>.
 * 
 * @author Anton Kabysh
 * @author Unknown - org.bouncycastle.crypto.prng.VMPCRandomGenerator
 * 
 */
final class VMPC extends CryptorandomnessEngine {
	...
	public final int read(ByteBuffer buffer) {
		...
		try {
			...
			while (buffer.hasRemaining()) {
				s = P[(s + P[n & 0xff]) & 0xff];
				buffer.put(P[(P[(P[s & 0xff]) & 0xff] + 1) & 0xff]);
				byte temp = P[n & 0xff];
				P[n & 0xff] = P[s & 0xff];
				P[s & 0xff] = temp;
				n = (byte) ((n + 1) & 0xff);
	...
	public byte nextByte() {
		try {
			...
			s = P[(s + P[n & 0xff]) & 0xff];
			return P[(P[(P[s & 0xff]) & 0xff] + 1) & 0xff];
	...
	public final int seedlen() {
		return CSPRNG.VMPC.seedlen.get();
	}
